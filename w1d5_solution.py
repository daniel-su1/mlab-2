...
if MAIN:
    ...
    def line(x_mean=(-3.0, 3.0), y_mean=(-3.0, 3.0), cov=(-0.99, 0.99)):
        ...
        """
        plt.rcParams["figure.figsize"] = (8, 5)  # type: ignore
        cov_matrix = np.eye(2) + np.array([[0, cov], [cov, 0]])
        data = np.random.multivariate_normal(
            np.array([x_mean, y_mean]), cov_matrix, 200
        )
...
# %%
r"""Their marginal distributions are the same! In both cases, all three variables have mean 0 ($E[X]=E[Y]=E[Z]=0$), and they both have all of their covariances $=0$, so $E[XYZ]=\kappa(X,Y,Z)$. But when we cut out those corners, $E[XYZ]$ will be positive, as these points dwell exclusively in the octants of $\mathbb R^3$ with positive product, whereas $E[XYZ]=0$ in the original distribution.
...
def isinstance_k(obj: Any) -> bool:
    ...
    return isinstance(obj, K)  # type: ignore
...
### Helper Functions
"""
# TBD lowpri: fix signatures here, problem is expr.args is Expr if expr is Mul, Add etc but expr.args is Basic if expr is just a Expr
...
def expand_exp(exp: Expr) -> Expr:
    ...
    def partition(lst):
        if len(lst) == 1:
            ...
            return
        first = lst[0]
        for smaller in partition(lst[1:]):
    ...
    assert isinstance_k(exp)
    arg = cast(Expr, exp.args[0])
    terms = monomial_to_list(arg)
    # # Last line of this func does this all at once
    # total = 0
    # for p in partition(terms):
    #   subtotal = 1
    #   for b in p:
    #     subtotal *= K(*sorted(b, key=str))
    #   total += subtotal
    # return total
    return Add(
        *map(
            lambda p: Mul(*map(lambda b: K_sort(*cast(list[Expr], b)), p)),
            partition(terms),
        )
    )
...
def bern_cumu(n: int) -> list[Expr]:
    ...
    for i in range(2, n + 1):
        terms = expand_exp(K(x**i)) - K(*([x] * i))
        # print(i, terms, cumus)
        cumus[K(*([x] * i))] = (p - sympy_map(lambda x: cumus[x], terms)).simplify()
    return list(cumus.values())
...
def prod_rule(cumu: Expr) -> Expr:
    """SOLUTION"""
    for i, arg in enumerate(cumu.args):
        if isinstance(arg, Pow) or isinstance(arg, Mul):
            # X is the first factor in the first arg, Y is the rest of the factors in this arg
            if isinstance(arg, Pow):
                A, B = arg.args[0], arg / arg.args[0]
            else:
                A, B = arg.args[0], Mul(*arg.args[1:])

            # all the other args in this cumulant
            S = list(cumu.args[:i] + cumu.args[(i + 1) :])

            # Find all bipartitions of S
            subsets = [([], S)]
            for el in S:
                # for each element of S, add the bipartitions formed by putting in it in each i so far.
                subsets += [
                    (l + [el], r[: r.index(el)] + r[r.index(el) + 1 :])
                    for (l, r) in subsets
                ]
                # print(subsets, S)

            total = K_sort(A, B, *cast(list[Expr], S))
            for (T, Tc) in subsets:
                total += K_sort(A, *T) * K_sort(B, *cast(list[Expr], Tc))
            # print("total:", total, "cumu was:", cumu)

            return sympy_map(prod_rule, total)
    return cumu
...
"""
# %%
# TBD lowpri: math notation above
...
def eval_gauss_bern_cumu(cumu: Expr) -> Expr:
    ...
    all_symbols = list(
        itertools.chain.from_iterable(
            map(monomial_to_list, cast(tuple[Expr], cumu.args))
        )
    )
    bern_cumus = bern_cumu(all_symbols.count(y))
    # print("bern cumus: ", bern_cumus)

    def subs(cumu):
        if set(cumu.args) == {x, y}:
            return 0
        elif set(cumu.args) == {x}:
            return int(len(cumu.args) == 2)
        else:
            return bern_cumus[len(cumu.args) - 1]

    # print("expansion", prod_rule(cumu))

    return sympy_map(subs, prod_rule(cumu))
...
def graph_solution(k: int) -> None:
    ...
    fig.add_scatter3d(
        x=line[:, 0], y=line[:, 1], z=line[:, 2], mode="lines", name="test"
    )
    fig.show()
