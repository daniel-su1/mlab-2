...
"""
# %%
# TBD lowpri: can we give an intuition for how much flattening/sharpening we get for a given temperature?
...
def sample_top_k(logits: t.Tensor, top_k: int) -> int:
    ...
    """
    "SOLUTION"
    # Alternate solution: set non-topk positions to -inf and sample from full distribution
...
if "SOLUTION":
    ...
    @dataclass
    class GPT2CacheEntry:
        """Mutable cache for one GPT2Block (specifically UnidirectionalAttention)."""

        # shape (batch, heads, seq, head_size)
        # seq is 0 if the cache is empty
        k: t.Tensor
        v: t.Tensor

        @classmethod
        def new_empty(cls, model: GPT2) -> list["GPT2CacheEntry"]:
            out = []
            for b in model.blocks:
                assert isinstance(b, GPT2Block)
                out.append(
                    cls(
                        k=t.empty(
                            (1, b.attn.num_heads, 0, b.attn.head_size),
                            device=b.attn.qkv_proj.weight.device,
                        ),
                        v=t.empty(
                            (1, b.attn.num_heads, 0, b.attn.head_size),
                            device=b.attn.qkv_proj.weight.device,
